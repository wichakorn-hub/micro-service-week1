...
int main() {
  ...
  if (const char *port_env = std::getenv("PORT"); port_env && *port_env) {
    try {
      ...
      std::cerr << "Invalid PORT env var: " << port_env << " (using 8010)"
                << std::endl;
      port = 8010;
    }
  }
  ...
  CROW_ROUTE(app, "/")([](const crow::request &req) {
    ...
    std::cout << "{\"code\":200,\"message\":\"OK\",\"method\":\"GET\",\"path\":"
                 "\"/\",\"trace_id\":\""
    ...
    x["level"] = "info";
    x["service"] = "cpp-api";
    x["message"] = std::string(crow::method_name(req.method)) + " " +
                   req.raw_url + " success";

    x["request"]["method"] = crow::method_name(req.method);
    x["request"]["url"] = req.raw_url;
    x["request"]["headers"]["user-agent"] = req.get_header_value("User-Agent");
    x["request"]["ip"] = req.remote_ip_address;

    x["response"]["status_code"] = 200;
    x["response"]["response_time_ms"] = 12;

    x["meta"]["request_id"] = trace_id;
    x["meta"]["user_id"] = 42;
